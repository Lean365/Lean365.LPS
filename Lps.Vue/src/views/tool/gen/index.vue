<template>
  <div class="app-container">
    ...
    <el-table ref="gridtable" v-loading="tableloading" :data="tableList" border
      ...
      <el-table-column prop="createTime" :label="$t('gencode.createTime')" sortable />
      <el-table-column prop="updateTime" :label="$t('gencode.updateTime')" sortable />
      <el-table-column :label="$t('btn.operation')" align="center" width="200">
        <template #default="scope">
          <el-button-group class="ml-4">
            <el-button type="info" plain size="small" icon="view" @click="handlePreview(scope.row)"
              v-hasPermi="['tool:gen:preview']" :title="$t('btn.preview')">
            </el-button>
            <el-button type="success" size="small" icon="edit" @click="handleEditTable(scope.row)"
              v-hasPermi="['tool:gen:edit']" :title="$t('btn.edit')">
            </el-button>

            <el-dropdown @command="handleCommand($event, scope.row)">
              <el-button color="#337ecc" size="small">
                ...
                <el-dropdown-menu>
                  <div v-hasPermi="['tool:gen:code']">
                    <el-dropdown-item command="generate">
                      <el-button icon="download" type="primary" link>{{ $t('btn.generateCode') }}</el-button>
                    </el-dropdown-item>
                  </div>
                  <div v-hasPermi="['tool:gen:edit']">
                    <el-dropdown-item command="sync">
                      <el-button icon="refresh" type="warning" link> {{ $t('btn.synchronize') }} </el-button>
                    </el-dropdown-item>
                  </div>
