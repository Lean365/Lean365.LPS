...
namespace Lps.ServiceCore.Service
{
    ...
    /// 文件管理
    /// </summary>
    [AppService(ServiceType = typeof(IOfficeFileService), ServiceLifetime = LifeTime.Transient)]
    public class OfficeFileService : BaseService<OfficeFile>, IOfficeFileService
    {
        private string domainUrl = AppSettings.GetConfig("ALIYUN_OSS:domainUrl");
        private readonly ISysConfigService SysConfigService;
        private OptionsSetting OptionsSetting;
        public OfficeFileService(ISysConfigService sysConfigService, IOptions<OptionsSetting> options)
        {
        ...
        /// <returns></returns>
        public async Task<OfficeFile> SaveFileToLocal(string rootPath, string fileName, string fileDir, string userName, IFormFile formFile)
        {
            ...
            OfficeFile file = new(formFile.FileName, fileName, fileExt, fileSize + "kb", filePath, userName)
            {
        ...
        /// <returns></returns>
        public async Task<OfficeFile> SaveFileToAliyun(OfficeFile file, IFormFile formFile)
        {
        ...
        public Task<long> InsertFile(OfficeFile file)
        {
            try
            {
